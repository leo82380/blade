...
public class Thunder : MonoBehaviour
{
    ...
    private IEnumerator DelayDamageCoroutine()
    {
        ...
        for(int i = 0; i < count; i++)
        {
            if (_colliders[i].TryGetComponent(out IDamageable health))
            {
                ...
                health.ApplyDamage(_skill.damage, point, Vector3.up, 0, _skill.player, DamageType.Projectile);
            }
        }
    ...
    {
        return Physics.OverlapSphereNonAlloc(transform.position, _skill.hitRadius, _colliders, _skill.whatIsEnemy);
    }
}
